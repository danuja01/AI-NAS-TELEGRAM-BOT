...
**CrowdSec security assistant** (`CROWDSEC_MONITOR_ENABLED=true`): polls `docker exec crowdsec cscli` (alerts, decisions, metrics), sends deduplicated Telegram alerts for meaningful attacks (SSH/HTTP brute force, exploits, scans), and a daily **NAS Security Daily Report**. `/crowdsec` shows a snapshot; `/security` runs the NAS Security Assistant persona. Requires a running `crowdsec` container and Docker socket access from the bot container. **Docker Compose:** `CROWDSEC_*` must appear under `environment:` in `docker-compose.yml` (host `.env` is not mounted into the container); recreate the bot after changing them.
