...
from utils.formatters import (
    format_system_stats, format_cpu_stats, format_memory_stats,
    format_disk_stats, format_temperature_stats, format_network_stats,
    format_health_score, format_smart_data, format_uptime, format_error
)
from services.system_monitor import (
    get_comprehensive_status, get_cpu_stats, get_memory_stats,
    get_disk_stats, get_temperatures, get_network_stats,
    get_uptime, calculate_health_score
)
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if 'error' in stats:
            await update.message.reply_text(format_error(stats['error']))
            return
        ...
        await update.message.reply_text(format_error(f"Failed to get system status: {e}"))
...
async def cpu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await save_conversation(user_id, 'assistant', message, command_output=f"CPU: {stats.get('percent', 0):.1f}%")
        await save_command(user_id, '/cpu', f"CPU: {stats.get('percent', 0):.1f}%")
        
    except Exception as e:
        logger.error(f"Error in cpu_command: {e}", exc_info=True)
        await update.message.reply_text(format_error(f"Failed to get CPU stats: {e}"))
...
async def ram_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(format_error(f"Failed to get memory stats: {e}"))
...
async def disk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(format_error(f"Failed to get disk stats: {e}"))
...
async def temps_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(format_error(f"Failed to get temperatures: {e}"))
...
async def network_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(format_error(f"Failed to get network stats: {e}"))
...
async def uptime_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        message = f"⏱ **System Uptime**\n\n{uptime_str}"
        await update.message.reply_text(message, parse_mode='Markdown')
        
        # Save to conversation history
        await save_conversation(user_id, 'user', '/uptime')
        await save_conversation(user_id, 'assistant', message, command_output=f"Uptime: {uptime_str}")
        await save_command(user_id, '/uptime', uptime_str)
        
    except Exception as e:
        logger.error(f"Error in uptime_command: {e}", exc_info=True)
        await update.message.reply_text(format_error(f"Failed to get uptime: {e}"))
...
async def health_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Save to conversation history
        await save_conversation(user_id, 'user', '/health')
        await save_conversation(user_id, 'assistant', message, 
        ...
        await update.message.reply_text(format_error(f"Failed to calculate health: {e}"))
...
async def smart_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.reply_text("💿 Reading SMART data...")
        ...
        await update.message.reply_text(format_error(f"Failed to get SMART data: {e}"))
